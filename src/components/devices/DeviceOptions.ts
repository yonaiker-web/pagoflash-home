type KeysOfType<T, F> = {
  [key in keyof T]: [F] extends [T[key]]
    ? [T[key]] extends [F]
      ? key
      : never
    : never;
...
type OptionField<T> = Omit<T, KeysOfSubType<T, undefined>> &
...
type DevicesType<
  R extends Record<string, DeviceType<string, readonly string[]>>
> = {
  [key in keyof R]: Compute<
    OptionField<
      OmitFieldByType<
        {
          device: key;
          color: R[key]["colors"][number];
          landscape: R[key]["hasLandscape"] extends true
            ? boolean | undefined
            : never;
          width?: number;
          height?: number;
